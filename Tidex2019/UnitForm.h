...
#pragma endregion
	private: System::Void Acceptbutton_Click(System::Object^ sender, System::EventArgs^ e) {
		if (extension == "hdf")
		{
			...
			PredictionDoneForm^ predictiondoneform = gcnew PredictionDoneForm(/*pasar por aqui la ruta .dat*/unitybox->Text); //se le pasa constructor la ruta del .dat generado y las unidades y se muestra gráfica;
		...
		else
		{
			...
			ChartForm^ predictiondoneform = gcnew ChartForm(/*pasar por aqui la ruta .dat*/unitybox->Text); //se le pasa constructor la ruta del .dat elegida y las unidades y se muestra gráfica;
			predictiondoneform->Show();
			this->Close();
